...
function manageDraw() {
  const continueWithDrawBreaker = checkPlayerHands(false);
  if (continueWithDrawBreaker) {
    let // Pull Three Cards From Each Player For The Pot
      p1Contribution = [
        pullACard(0),
        pullACard(0),
        pullACard(0)
      ...
      p1DrawBreaker = pullACard(0),
      p2DrawBreaker = pullACard(1);

    game.liveDetails.drawPot = game.liveDetails.drawPot.concat(p1Contribution, p2Contribution);

    console.log('DRAW: p1Contribution ', p1Contribution);
    console.log('DRAW: p2Contribution ', p2Contribution);

    console.log('DRAW: p1Card ', p1DrawBreaker);
    console.log('DRAW: p2Card ', p2DrawBreaker);

    const
      { roundId } = getCurrentRoundInformation(),
      p1CardValue = p1DrawBreaker.cardValue,
      p2CardValue = p2DrawBreaker.cardValue,
      player1Wins = p1CardValue > p2CardValue,
      player2Wins = p2CardValue > p1CardValue,
      draw = p1CardValue === p2CardValue,
      score_value = draw ? (p1CardValue + p2CardValue) : player1Wins ? p1CardValue : (p1CardValue + p2CardValue),
      histolog = {
        player1Wins, player2Wins, draw, score_value
      };

    game.liveDetails.hist[roundId].histolog.drawHistolog = histolog;
    if (draw) {
      return manageDraw();
    } else {
      // Winner Needs cards from all pots...
      return routeWinner();
    }
  }
}
